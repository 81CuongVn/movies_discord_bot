...
module.exports.run = async (bot, message, args, serverData) => {
    ...
    for (const tempMovie of serverData.movies) {
        moviesRes.push(tempMovie.movieID);
    }
        ...
        try {
            ...
            collector.on('collect', async (reaction, user) => {
                if (reaction.emoji.name == '⬅️') {
                    ...
                    if (moviesRes.includes(moviePages[page - 1].imdbID)) {
                        message.channel.send("Movie has already been added.");
                    ...
                    try {
                        let movieObj = {
                            movieTitle: moviePages[page - 1].title,
                            movieID: moviePages[page - 1].imdbID,
                            addedBy: message.author.id,
                            dateAdded: Date.now()
                        };
                        const response = await serverModels.findOneAndUpdate(
                            {
                        ...
                        message.channel.send("Movie successfully added.");
                    } catch (err) {
                        console.log(err);
                        message.channel.send("Movie could not be added, try using an IMDb link.");
                    }
                    ...
                    embed
                        .setTitle(moviePages[page - 1].title)
                        .setDescription(moviePages[page - 1].description).setImage(moviePages[page - 1].image)
                        .setAuthor(moviePages[page - 1].actors)
                        .addField("Genre", moviePages[page - 1].genre, true)
                        .addField("Runtime", moviePages[page - 1].runtime, true)
                        .addField("Release", moviePages[page - 1].release, true)
